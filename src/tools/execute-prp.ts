...
export const ExecutePRPInputSchema = z.object({
  prpContent: z.string().min(100).describe('The PRP content to analyze for execution guidance'),
  focusAreas: z.array(z.enum(['agents', 'tasks', 'risks', 'technology', 'timeline', 'phases']))
  ...
  includeTechnologyRec: z.boolean().default(true)
    .describe('Include technology stack recommendations'),
  targetTeamSize: z.number().int().min(1).max(20).optional()
  ...
  }).optional().describe('Additional project constraints to consider'),
});
...
export async function executePRPToolHandler(args: unknown): Promise<{
  ...
  try {
    ...
    const guidance = await executionGuidance.generateExecutionGuidance(input.prpContent);
    ...
    // Timeline and phases
    if (input.focusAreas.includes('timeline') || input.focusAreas.includes('phases')) {
    ...
    if (input.detailLevel === 'comprehensive') {
      result.executiveSummary = generateExecutiveSummary(guidance, input.projectConstraints);
    }
...
function formatTaskBreakdown(taskGroups: any[], detailLevel: string): any {
  ...
  const totalHours = taskGroups.reduce((sum, group) => sum + group.estimatedHours, 0);
  ...
  const formatted = {
    ...
    taskGroups: taskGroups.map(group => ({
      ...
      dependencies: detailLevel === 'comprehensive' ? group.dependencies : undefined,
      taskCount: group.tasks.length,
      tasks: detailLevel === 'comprehensive' ? group.tasks.map(task => ({
        id: task.id,
        title: task.title,
      ...
      sampleTasks: detailLevel === 'detailed' ? group.tasks.slice(0, 3).map(task => ({
        title: task.title,
        complexity: task.complexity,
        estimatedHours: task.estimatedHours,
      })) : undefined,
...
function formatRiskAnalysis(riskAssessment: any, detailLevel: string): any {
  const formatted: any = {
    summary: {
      ...
      riskDistribution: {
        critical: riskAssessment.risks.filter((r: any) => r.severity === 'critical').length,
        high: riskAssessment.risks.filter((r: any) => r.severity === 'high').length,
        medium: riskAssessment.risks.filter((r: any) => r.severity === 'medium').length,
        low: riskAssessment.risks.filter((r: any) => r.severity === 'low').length,
      }
    },
    topRisks: riskAssessment.risks
      ...
      .slice(0, detailLevel === 'summary' ? 3 : detailLevel === 'detailed' ? 5 : 10)
      .map((risk: any) => ({
        id: risk.id,
        ...
        mitigation: detailLevel === 'comprehensive' ? risk.mitigation : undefined,
      })),
    mitigationStrategies: detailLevel !== 'summary' ?
      riskAssessment.mitigationStrategies.slice(0, detailLevel === 'comprehensive' ? 10 : 5) :
      undefined,
  };
...
function formatTechnologyRecommendations(
  ...
  const formatted: any = {
    ...
    byCategory: Object.entries(categoryGroups).map(([category, technologies]: [string, any]) => ({
      category,
      recommendations: technologies.map((tech: any) => ({
        ...
        alternatives: detailLevel === 'comprehensive' ? tech.alternatives : undefined,
        preferred: preferredTech?.includes(tech.technology.toLowerCase()) || false,
      }))
    }))
  };
  ...
  if (preferredTech && preferredTech.length > 0) {
    const alignedTech = techStack.filter(tech =>
      preferredTech.some(preferred => tech.technology.toLowerCase().includes(preferred.toLowerCase()))
    );

    formatted.preferredTechAlignment = {
      alignmentScore: Math.round((alignedTech.length / preferredTech.length) * 100),
      alignedTechnologies: alignedTech.map(tech => tech.technology),
      missingPreferred: preferredTech.filter(preferred =>
        !techStack.some(tech => tech.technology.toLowerCase().includes(preferred.toLowerCase()))
      ),
    };
  }
...
function formatImplementationPlan(
  ...
  const formatted = {
    ...
    phases: phases.map(phase => ({
      ...
      dependencies: detailLevel === 'comprehensive' ? phase.dependencies : undefined,
      deliverables: detailLevel !== 'summary' ? phase.deliverables : undefined,
      taskGroups: detailLevel === 'comprehensive' ?
        phase.taskGroups.map((tgId: string) => {
    ...
    criticalPath: {
      ...
      totalDuration: criticalPath.reduce((sum, phase) => sum + phase.estimatedDuration, 0),
    }
  };
 ...
 */
function generateLocomotiveRecommendations(guidance: any, targetTeamSize?: number): any {
  const totalAgents = guidance.agentRecommendations.reduce((sum: number, agent: any) => sum + agent.count, 0);

  const recommendations = {
    orchestrationStrategy: determineOrchestrationStrategy(guidance.estimatedComplexity, totalAgents),
    agentDeployment: {
      concurrent: Math.min(totalAgents, targetTeamSize || totalAgents),
      sequential: totalAgents > (targetTeamSize || 10),
      specialization: guidance.agentRecommendations.slice(0, 3).map((agent: any) => ({
    ...
    monitoringRecommendations: generateMonitoringRecommendations(guidance.riskAssessment),
  };
...
function generateKeyFindings(guidance: any): string[] {
  ...
  findings.push(`Project complexity assessed as ${guidance.estimatedComplexity}`);
  findings.push(`Estimated duration: ${guidance.estimatedDuration.minimum}-${guidance.estimatedDuration.maximum} ${guidance.estimatedDuration.unit}`);
  findings.push(`Overall risk level: ${guidance.riskAssessment.overallRisk}`);
  findings.push(`Recommended team size: ${guidance.agentRecommendations.reduce((sum: number, agent: any) => sum + agent.count, 0)} agents`);

  const criticalRisks = guidance.riskAssessment.risks.filter((r: any) => r.severity === 'critical').length;
  if (criticalRisks > 0) {
    findings.push(`${criticalRisks} critical risks identified requiring immediate attention`);
  }
...
function generateExecutiveRecommendations(guidance: any): string[] {
  ...
  if (guidance.riskAssessment.overallRisk === 'high' || guidance.riskAssessment.overallRisk === 'critical') {
    recommendations.push('Invest in comprehensive risk mitigation before starting development');
  }

  recommendations.push('Implement continuous monitoring and regular milestone reviews');

  const topAgent = guidance.agentRecommendations[0];
  if (topAgent) {
    recommendations.push(`Prioritize ${topAgent.agentType} agent deployment for early wins`);
  }
...
function identifySuccessFactors(guidance: any): string[] {
  ...
  if (guidance.estimatedComplexity === 'low' || guidance.estimatedComplexity === 'medium') {
...
function identifyPotentialChallenges(guidance: any): string[] {
  ...
  if (guidance.riskAssessment.risks.length > 5) {
    challenges.push('Multiple risks require careful coordination and monitoring');
  }

  const totalHours = guidance.taskBreakdown.reduce((sum: number, group: any) => sum + group.estimatedHours, 0);
  if (totalHours > 400) {
...
function analyzeConstraints(guidance: any, constraints: any): any {
  ...
  if (constraints.timeline) {
    ...
    const constraintWeeks = parseInt(constraints.timeline.replace(/\D/g, '')) || estimatedWeeks;

    if (estimatedWeeks > constraintWeeks) {
      analysis.feasibility = 'CHALLENGING';
      analysis.concerns.push(`Timeline constraint (${constraintWeeks} weeks) is tighter than estimate (${estimatedWeeks} weeks)`);
      analysis.recommendations.push('Consider reducing scope or increasing team size');
    }
  }
...
function determineOrchestrationStrategy(complexity: string, agentCount: number): any {
  if (complexity === 'very-high' || agentCount > 8) {
    ...
    return {
      type: 'collaborative',
      description: 'Direct collaboration between agents with shared coordination',
...
function generateMonitoringRecommendations(riskAssessment: any): string[] {
  ...
  if (riskAssessment.overallRisk === 'high' || riskAssessment.overallRisk === 'critical') {
    recommendations.push('Establish weekly risk review meetings with stakeholders');
    recommendations.push('Implement early warning systems for critical risk indicators');
  }

  recommendations.push('Create cross-agent communication protocols and shared knowledge base');
